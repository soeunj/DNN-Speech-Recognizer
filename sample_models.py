...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for l in range(recur_layers):
        rnn = GRU(units, activation='relu',
                  ...
                  name='rnn_'+str(l))(input_data)
        batch = BatchNormalization(name='bn_rnn_'+str(l))(rnn)
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
...
def final_model(input_dim, filters, kernel_size, conv_stride,
    ...
    conv_1d = Conv1D(filters, kernel_size,
                     ...
                     padding=conv_border_mode,
                     activation='relu',
                     name='conv_d')(input_data)
    
    bn_cnn = BatchNormalization(name='bn1')(conv_1d)

    for l in range(recur_layers):
        rnn = Bidirectional(GRU(units, return_sequences=True,
                                name='bi_rnn_'+str(l),
                                implementation=2))(bn_cnn)
        bnn = BatchNormalization(name='bn_rnn_'+str(l))(rnn)
        bn_cnn = Dropout(0.2)(bnn)
